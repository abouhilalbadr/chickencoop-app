...
<template>
  ...
  <div class="p-2">
    ...
    <div class="mb-4 mx-2 flex flex-col gap-2 border-t border-border pt-4">
      <span class="text-black/40 font-bold">Type</span>
      <div class="grid grid-cols-2 gap-2 mb-4">
        <button v-for="(item, i) in types" :key="i" class="border border-main rounded-md py-3 text-sm"
          :class="item.value === type ? 'bg-main text-white' : 'text-main bg-white'" @click="changeType(item.value)">
          {{ item.name }}
        </button>
        <button class="border border-main rounded-md py-3 text-sm col-span-2"
          :class="type === 'GLOVO' ? 'bg-main text-white' : 'text-main bg-white'" @click="changeType('GLOVO')">
          Glovo
        </button>
      </div>
    </div>
    <div v-if="type === 'LIVRAISON'" class="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-6 px-2">
      <button @click="livraison = 0" class="border border-main rounded-md"
        :class="livraison === 0 ? 'bg-main text-white' : 'text-main bg-white'">Gratuit</button>
      <button @click="livraison = 10" class="border border-main rounded-md"
        :class="livraison === 10 ? 'bg-main text-white' : 'text-main bg-white'">Zone A</button>
      <button @click="livraison = 20" class="border border-main rounded-md"
        :class="livraison === 20 ? 'bg-main text-white' : 'text-main bg-white'">Zone B</button>
    </div>
    <div v-if="type === 'GLOVO'" class="grid grid-cols-2 gap-6 mb-6 px-2">
      <button @click="payType = 'CASH'" class="border border-main rounded-md py-2"
        :class="payType === 'CASH' ? 'bg-main text-white' : 'text-main bg-white'">Espèce</button>
      <button @click="payType = 'CARD'" class="border border-main rounded-md py-2"
        :class="payType === 'CARD' ? 'bg-main text-white' : 'text-main bg-white'">Carte bancaire</button>
    </div>
    <div v-if="type === 'GRATUIT'">
      <div v-if="cart.length > 0" class="mb-4 mx-2 flex items-center justify-between gap-2">
        ...
        <input type="text" placeholder="Nom"
          class="border border-border rounded-lg p-2 h-10 text-lg text-main outline-none" v-model="freeUser">
      </div>
    </div>
    <div v-if="type !== 'GRATUIT'">
      <div v-if="cart.length > 0" class="mb-4 mx-2 flex items-center justify-between gap-2 border-t border-border pt-4">
        ...
        <span class="text-xl font-bold font-bree-serif text-main">{{ subTotal }} <span
            class="text-base">DH</span></span>
      </div>
      <div v-if="cart.length > 0" class="mb-4 mx-2 flex items-center justify-between gap-2">
        <span class="text-black/40 font-bold">Payé</span>
        <input type="text" placeholder="0" class="border border-border rounded-lg p-2 h-10 text-lg text-main"
          v-model="pay">
      </div>
      <div v-if="showPromo && cart.length > 0" class="mb-2 mx-2 flex items-center justify-between gap-2">
        ...
        <span class="text-2xl font-bold font-bree-serif text-main"> - {{ parseFloat(percentTotal).toFixed(2) }}<span
            class="text-base">DH</span></span>
      </div>
      <div v-if="type === 'LIVRAISON'" class="mb-2 mx-2 flex items-center justify-between gap-2">
        ...
        <span class="text-2xl font-bold font-bree-serif text-main">{{ livraison }} <span
            class="text-base">DH</span></span>
      </div>
      <div v-if="cart.length > 0" class="mb-2 mx-2 flex items-center justify-between gap-2">
        ...
        <span class="text-2xl font-bold font-bree-serif text-main">{{ parseFloat(pay - ((subTotal + (livraison === -1 ?
          0 : livraison)) - percentTotal)).toFixed(2) }} <span class="text-base">DH</span></span>
      </div>
      <div v-if="cart.length > 0" class="mb-4 mx-2 flex items-center justify-between gap-2 border-t border-border pt-4">
        <span class="text-xl text-black/40 font-bold">Total</span>
        <span class="text-2xl font-bold font-bree-serif text-main">{{ parseFloat((subTotal + (livraison === -1 ? 0 :
          livraison)) - percentTotal).toFixed(2) }} <span class="text-base">DH</span></span>
      </div>
